    ...
    if (result.success && result.url) {
      ...
      try {
        ...
        savedImageData = await saveGeneratedImage(
          { url: result.images[0].url, width: result.images[0].width, height: result.images[0].height },
          user,
          generation
        );
        ...
        if (savedImageData) {
          ...
          try {
            const imgbbUrl = await uploadToImgbb(result.images[0].url);
      ...
      const responseData = {
        success: true,
        image: savedImageData?.url || result.images[0].url,
        thumbnailUrl: savedImageData?.thumbnailUrl || result.images[0].url,
        credits: {
          used: creditsUsed,
