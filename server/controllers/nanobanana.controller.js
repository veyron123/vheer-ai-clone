...
export const generateImage = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (USE_FAL_AI) {
      ...
      console.log('🔍 [FAL.AI] Raw response from Fal.ai (regular):', JSON.stringify(falResult, null, 2));
      console.log('✅ [FAL.AI] Generation completed');
      
      if (!falResult || typeof falResult !== 'object') {
        console.error('❌ [FAL.AI] Invalid response format (regular):', falResult);
      ...
      if (!images || !Array.isArray(images) || images.length === 0) {
        console.error('❌ [FAL.AI] No images in response (regular). Available keys:', Object.keys(falResult));
        console.error('❌ [FAL.AI] Images field (regular):', images);
        console.error('❌ [FAL.AI] Data field (regular):', falResult.data);
